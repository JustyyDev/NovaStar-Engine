...
class AutoUpdater {
  ...
  async installUpdate() {
    try {
      if (!this._downloadPath || !fs.existsSync(this._downloadPath)) return false;

      const result = await dialog.showMessageBox({
        type: 'question', title: 'Install Update',
        message: 'The update is ready. The app will close and the installer will run.',
        buttons: ['Install Now', 'Later'], defaultId: 0, cancelId: 1,
      });

      if (result.response !== 0) return false;

      spawn(this._downloadPath, ['/S'], { detached: true, stdio: 'ignore' }).unref();
      app.quit();
      return true;
    } catch (e) {
