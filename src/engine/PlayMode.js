...
export class PlayMode {
  ...
  enter(editorEntities, novaScript = null) {
    ...
    // Check if there's a camera entity (first-person mode)
    const cameraEntity = this._runtimeEntities.find(e => e.isCamera);
    if (cameraEntity && this.engine.fpsController) {
      this._fpsMode = true;
      const pos = cameraEntity.mesh.position;
      this.engine.fpsController.setPosition(pos.x, pos.y + 1.6, pos.z);
      this.engine.fpsController.enable();
    } else {
      this._fpsMode = false;
    }
  ...
  exit(editorEntities) {
    ...
    // Disable FPS controller if it was active
    if (this._fpsMode && this.engine.fpsController) {
      this.engine.fpsController.disable();
      this._fpsMode = false;
    }
  ...
  _update(dt) {
    ...
    // Update FPS controller if in first-person mode
    if (this._fpsMode && this.engine.fpsController) {
      this.engine.fpsController.update(dt);
    }
