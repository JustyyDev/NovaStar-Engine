/**
 * ╔══════════════════════════════════════════╗
 *  NOVASTAR ENGINE v0.4.1 — Core Runtime
 *  A versatile 3D game engine supporting
 *  cartoony, realistic, and stylized games
 *  + PBR Materials, Project Folders, Play Mode
 * ╚══════════════════════════════════════════╝
 */
...
import { RaycastInteraction } from './Raycaster.js';
...
export class NovaStarEngine {
  ...
  _init() {
    ...
    this.save = new SaveSystem(this.options.gameId || 'novastar');
    ...
    this.exporter = new GameExporter(this);
    this.projectFolder = new ProjectFolder();
    this.fpsController = new FirstPersonController(this);
    this.interaction = new RaycastInteraction(this);
    this.sprites = new SpriteSystem(this);

    // Handle resize
    window.addEventListener('resize', () => this._onResize());

    console.log(
      '%c⭐ NovaStar Engine v0.5.0 initialized',
