/**
 * NovaStar Editor Application v0.1.0
 * Visual scene editor with template system
 */

import * as THREE from 'three';

// Lazy imports - these load only when the editor actually starts
let NovaStarEngine, MeshBuilder, Interpreter;

async function loadEngineModules() {
  const engineMod = await import('../engine/NovaStarEngine.js');
  NovaStarEngine = engineMod.NovaStarEngine;
  const builderMod = await import('../engine/MeshBuilder.js');
  MeshBuilder = builderMod.MeshBuilder;
  try {
    const interpMod = await import('../novascript/Interpreter.js');
    Interpreter = interpMod.Interpreter;
  } catch (e) {
    console.warn('NovaScript failed to load:', e);
    Interpreter = null;
  }
}
...
class EditorApp {
  constructor() {
    // Don't init until a template is chosen
    this.engine = null;
    this.builder = null;
  ...
  async _startEditor(template) {
    ...
    try {
      this.novaScript = Interpreter ? new Interpreter(this.engine) : null;
    } catch (e) {
      console.warn('NovaScript init warning:', e);
      this.novaScript = null;
    }
