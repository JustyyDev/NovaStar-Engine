/**
 * NovaStar Editor Application v0.1.3
...
import { Interpreter } from '../novascript/Interpreter.js';
...
class EditorApp {
  ...
  _setupWelcome() {
    // Use event delegation on the whole welcome screen so it works even if cards load late
    ...
    welcome.addEventListener('click', (e) => {
      ...
      if (template) this._startEditor(template);
    });
  }
...
function boot() {
  try {
    ...
    window.editor = app;
    console.log('[NovaStar] Editor app created successfully');
  } catch (e) {
    console.error('[NovaStar] Failed to create editor:', e);
  }
}
